...
菜单 `1` 是推荐给新用户的极简小白模式；菜单 `2` 是需要完整自定义时使用的高级
一条路流程。

小白视角可以先记住一句话：如果脚本能自动识别 Fractald，并且 RPC/ZMQ 检查通过，
后续大部分会按默认值自动完成；如果识别成功，极简模式通常只问 RPC 密码和最终部署
确认。如果识别失败，主要需要在高级模式里手动补 Fractald 的 RPC/ZMQ 地址和 RPC
用户名/密码。更简短的说明见 [小白使用难度说明](BEGINNER.zh-CN.md)。
